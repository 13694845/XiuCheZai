...
@interface XCZCircleDetailALayerRow() <XCZCircleDetailALayerRowReplyViewDelegate>
...
@implementation XCZCircleDetailALayerRow
...
- (void)setupView {
    ...
    NSString *yreply_content = [_remark[@"reply_content"] stringByReplacingOccurrencesOfString:@"#0A;" withString:@"\n"];
    NSAttributedString *attributeStr = [self changeRichText:yreply_content];
    ...
    reply_contentLabel.frame = CGRectMake(XCZNewDetailRemarkRowMarginX + 33 + XCZNewDetailRemarkRowMarginX, self.height + XCZNewDetailRemarkRowMarginY * 2, reply_contentLabelSize.width + self.touxiangCount * 14, reply_contentLabelSize.height);
    ...
    if (reply_infoArray.count) {
        for (int i = 0; i<reply_infoArray.count; i++) {
            XCZCircleDetailALayerRowReplyView *subViewRow = [[XCZCircleDetailALayerRowReplyView alloc] init];
...
- (void)circleDetailALayerRowReplyView:(XCZCircleDetailALayerRowReplyView *)circleDetailRemarkRowReplyView nameDidClickWithUserId:(NSString *)bbs_user_id
{
    if ([self.delegate respondsToSelector:@selector(detailALayerRowReplyView:nameDidClickWithUserId:)]) {
        [self.delegate detailALayerRowReplyView:circleDetailRemarkRowReplyView nameDidClickWithUserId:bbs_user_id];
    }
}
