...
@implementation XCZCircleDetailRemarkRow
...
- (void)setupView {
    ...
    if (reply_infoArray.count) {
        for (int i = 0; i<reply_infoArray.count; i++) {
            if (i<2) { // 回复内容这里只显示两条
                XCZCircleDetailRemarkRowReplyView *subViewRow = [[XCZCircleDetailRemarkRowReplyView alloc] init];
...
- (void)circleDetailRemarkRowReplyView:(XCZCircleDetailRemarkRowReplyView *)circleDetailRemarkRowReplyView nameDidClickWithUserId:(NSString *)bbs_user_id
{
    if ([self.delegate respondsToSelector:@selector(detailRemarkRowReplyView:nameDidClickWithUserId:)]) {
        [self.delegate detailRemarkRowReplyView:circleDetailRemarkRowReplyView nameDidClickWithUserId:bbs_user_id];
    }
}
