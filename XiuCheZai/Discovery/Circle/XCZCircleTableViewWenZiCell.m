...
@interface XCZCircleTableViewWenZiCell()
...
@implementation XCZCircleTableViewWenZiCell
...
- (void)setupAttrAndFrame
{
    ...
    if (!addr.length) {
        ...
        self.headerCityLabel.text = [NSString stringWithFormat:@"%@ · %@", _row[@"user_forum_name"], addr];
    }
    ...
    self.cellContentView.frame = CGRectMake(0, CGRectGetMaxY(self.cellHeaderView.frame), self.selfW, CGRectGetMaxY(self.contentLabel.frame) + 8 + 28);
    self.bottomLineView.frame = CGRectMake((CGRectGetMaxX(self.headerImageView.frame) + 8), self.cellContentView.bounds.size.height - 1, self.selfW - (CGRectGetMaxX(self.headerImageView.frame) + 8), 1);
    CGFloat cellWZHeight = CGRectGetMaxY(self.cellContentView.frame);
...
{
    if ([self.delegate respondsToSelector:@selector(circleTableViewWenZiCell:cellHeaderViewDidClick:)]) {
        [self.delegate circleTableViewWenZiCell:self cellHeaderViewDidClick:self.row];
    }
}
