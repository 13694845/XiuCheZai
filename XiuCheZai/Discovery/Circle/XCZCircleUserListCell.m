...
@interface XCZCircleUserListCell()
...
@implementation XCZCircleUserListCell
...
- (void)setRow:(NSDictionary *)row
{
    ...
    if (![_row[@"user_id"] isEqualToString:self.tieziUser_id]) {
...
{
    if ([self.delegate respondsToSelector:@selector(circleUserListViewCell:siteCircleLabelDidClick:)]) {
        [self.delegate circleUserListViewCell:self siteCircleLabelDidClick:[_row[@"is_guan"] intValue]];
    }
}
